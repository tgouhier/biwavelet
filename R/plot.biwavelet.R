...
#'   vectors using \link{colorRampPalette} or \link{brewer.pal} from package
#'   \link{RColorBrewer}. Value NULL generates MATLAB's jet color palette.
#' @param xlab Xlabel of the figure.
#' @param ylab Ylabel of the figure.
...
#'   regions are desired, then \code{tol} must be set to \code{1}.
#' @param plot.cb Plot color bar if TRUE.
...
#'   thewavelet coefficients, or \code{phase} to plot the phase.
#' @param plot.coi Plot cone of influence (COI) as a semi-transparent polygon if
#'   TRUE. Areas that fall within the polygon can be affected by edge effects.
...
#'   transparency).
#' @param plot.sig Plot contours for significance if TRUE.
...
#' @param bw plot in black and white if TRUE.
#' @param legend.loc Legend location coordinates as defined by
#'   \code{image.plot}.
#' @param legend.horiz Plot a horizontal legend if TRUE.
...
#'   \code{arrow.cutoff} for \link{wt} and \link{xwt} objects. For \link{pwtc}
#'   and \link{wtc} objects, phase arrows will be plotted in regions where the
#'   \code{rsq} value exceeds \code{arrow.cutoff}. If the object being plotted
#'   does not have a significance field, regions whose z-values exceed the
#'   \code{arrow.cutoff} quantile will be plotted.
...
#' See \link{Date} for other valid formats.
