...
test_that("Quantile out of range should fail", {
  ...
  expect_error(rowQuantileC(data, 2), regexp = "out of range")
  expect_error(rowQuantileC(data, -1), regexp = "out of range")
  expect_error(rowQuantileC(data, 1.1), regexp = "out of range")
...
test_that("R and c++ version work the same on small inputs", {
  ...
  for (data in list(
    ...
    expect_equal(rowQuantileC(data, .2),
